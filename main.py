...
from FLAlgorithms.servers.serverpFedCdFg import FedCdFg
...
def create_server_n_user(args, i):
    ...
    elif 'FedCdFg' in args.algorithm:
        server=FedCdFg(args, model, i)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--min_loss_threshold', type=float, default=0.5,
                      help='Minimum loss threshold for storing samples')
    parser.add_argument('--memory_sample_ratio', type=float, default=0.5,
                      help='Ratio of memory samples in each batch')
