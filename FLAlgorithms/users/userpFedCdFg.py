...
class UserpFedCdFg(User):
    ...
    def Pretrain(self, user_id, cluster_models, glob_iter, personalized=False, early_stop=100, regularization=True, verbose=False):
    ...
    def train(self, user_id, cluster_models,cluster_weights, glob_iter, personalized=False, early_stop=100, regularization=True, verbose=False):
        self.clean_up_counts()
        self.model.train()
        self.generative_model.eval()
        ...
        for epoch in range(self.local_epochs):
            for i in range(self.K):
                ...
                if regularization and epoch < early_stop:
                    ...
                    output_given_gen=self.model(gen_output, start_layer_idx=self.latent_layer_idx, logit=True)['output']
